...
class ExpVQVAE(ExpBase):
    ...
    def forward(self, batch):
        ...
        x_masked = x.clone()
        x_masked[:,:,-1] = torch.zeros(x_masked.shape[0], x_masked.shape[1])
        ...
        u_l_masked = zero_pad_high_freq(time_to_timefreq(x_masked, self.n_fft, C))
        ...
        u_h_masked = zero_pad_low_freq(time_to_timefreq(x_masked, self.n_fft, C))

        z_h = self.encoder_h(u_h_masked)
